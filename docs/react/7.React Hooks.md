...
- It does not cause a re-render at all.
  ...
  // now expensive Task will only run when the value of 'number' is changed
    ...
    return (
      <div>
        ...
        value={number}
        // input values are always strings, so convert to a number
...
When state grows, you can split logic into smaller reducer functions and delegate to them from a single root reducer based on the slice of state being updated:
...
// Usage: useReducer with the combined reducer
...
Each sub-reducer manages its own slice and ignores actions it doesn't recognize (returning its slice unchanged). For larger apps, libraries like Redux Toolkit provide `combineReducers` to automate this.
...
```jsx
// using external store in this component using useSyncExternalStore hook
"use client";
...
!!! note "React 19: `ref` is now a regular prop"
    As of **React 19**, function components can receive `ref` as a normal prop, and **`forwardRef` is deprecated** (a codemod is provided to migrate). The example below uses `forwardRef` for compatibility with React 18 and earlier. In React 19 you can write the child as:

    ```jsx
    function CustomInput({ ref, ...props }) {
      const inputRef = useRef(null);
      useImperativeHandle(ref, () => ({ /* exposed methods */ }));
...
function useLocalStorage(key, initialValue) {
  const [storedValue, setStoredValue] = useState(() => {
    // localStorage only stores strings, so parse JSON when reading
    const item = localStorage.getItem(key);
    return item != null ? JSON.parse(item) : initialValue;
  });

  useEffect(() => {
    // ...and stringify when writing, otherwise objects become "[object Object]"
