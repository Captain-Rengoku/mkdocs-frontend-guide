...
- **Understanding Stale State Issue**
  ...
  function Counter() {
    ...
    const handleIncrement = () => {
      ...
      // `count` is captured once for this render. All three calls below read the
      // SAME stale value, so they all compute the same result (count + 1).
      setCount(count + 1); // schedules: 0 + 1 = 1
      setCount(count + 1); // schedules: 0 + 1 = 1 (still the stale 0)
      setCount(count + 1); // schedules: 0 + 1 = 1
      // React batches these into one re-render, and the last write wins.
  ...
  - When `handleIncrement` runs, `setCount(count + 1)` is called three times.
  - Since `count` holds the same stale value (`0`) in every call, they all set it to `1` instead of `3`.
...
- Also no need for event binding at all.
...
- **Expensive Computation without Memoization**
  ...
  function LargeList({ items }) {
    // ⚠️ items.sort() mutates the original `items` prop in place!
    // Always copy first with [...items] before sorting.
...
- Also note: `Array.prototype.sort()` mutates the array it is called on. Calling it directly on a prop (`items.sort(...)`) mutates the parent's data, which can cause subtle bugs. Spread into a new array first: `[...items].sort(...)`.
