...
```jsx
{/* becomes /dashboard/settings if inside dashboard */}
...
The trailing `?` makes a **dynamic** param optional — the route matches both `/settings` and `/settings/profile`, with `tab` being `undefined` in the first case:

```jsx
<Route path="/settings/:tab?" element={<Settings />} />
```

### Multiple Segments with an Optional Param

Here `:id` is required and `:mode` is an optional dynamic param:
...
| `<ScrollRestoration />` | A component (render once in your root layout) that auto-restores scroll.   |
