...
Testing forms confirms that inputs update, validation fires, and submission produces the right data. The modern stack is **React Testing Library (RTL)** for rendering and queries, **`@testing-library/user-event`** for realistic interactions, and a runner like **Vitest** or **Jest**.
...
The test APIs are identical across runners — only the config and imports differ. Use `describe`/`it`/`expect` from `vitest`, or rely on Jest's globals.

### 1. Unit Testing Form Logic

Pure validation helpers can be tested without rendering anything:
...
### 2. Integration Testing a Controlled Form with React Testing Library

Given a simple controlled login form:
...
A test using `user-event` to type and submit:

```jsx
// LoginForm.test.jsx
import { describe, it, expect, vi } from "vitest"; // for Jest: const vi = jest;
...
describe("LoginForm", () => {
  it("submits the typed values", async () => {
    const handleSubmit = vi.fn(); // jest.fn() with Jest
...
- Query by accessible roles/labels (`getByLabelText`, `getByRole`) rather than test IDs — this also verifies accessibility.
- Use `await user.type(...)` / `await user.click(...)`; `user-event` is async.
- Prefer `findBy*` (async) for content that appears after an interaction.

### 3. Testing a React Hook Form Form

React Hook Form is tested the same way — interact through the UI; you do not need to mock the library. Note its validation runs asynchronously, so assert with `findBy*`.

```jsx
// RHFLogin.jsx
import { useForm } from "react-hook-form";

export default function RHFLogin({ onSubmit }) {
  const {
    register,
    handleSubmit,
    formState: { errors },
...
describe("RHFLogin", () => {
  it("shows a required error when submitting empty", async () => {
    ...
    const user = userEvent.setup();
    render(<RHFLogin onSubmit={onSubmit} />);
  ...
  it("calls onSubmit with valid data", async () => {
    ...
    // RHF passes (data, event) to the submit handler
...
### 4. Mocking Form Events & API Calls

When a form submits to an API, mock the network call so tests stay fast and deterministic:

```jsx
import { vi } from "vitest"; // jest.spyOn with Jest
...
Snapshots catch unintended markup changes; keep them small and focused:
