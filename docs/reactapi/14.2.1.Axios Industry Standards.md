...
### 2️⃣ Axios Interceptors (Auth, Logging, Error Handling)
...
!!! warning "Token in localStorage is shown for simplicity only"
    This example (and the `axiosClient.js` setup later in this chapter) reads the
    JWT from `localStorage`. That is convenient, but `localStorage` is accessible
    to any JavaScript on the page, so a single XSS flaw lets an attacker exfiltrate
    the token. For production, prefer an **HttpOnly, Secure, SameSite cookie** set
    by the server (send it automatically with `withCredentials: true`), since
    JavaScript cannot read it. See the Authentication chapter for details.
...
### 5️⃣ Retry Failed Requests with Exponential Backoff
...
### 1️⃣1️⃣ Cache + LocalStorage Fallback
...
axiosClient.interceptors.response.use(
  (response) => response.data, // ⚠️ Unwraps the body: callers get `data` directly, not the full AxiosResponse
  (error) => {
...
!!! note "This interceptor unwraps the response"
    Because the response interceptor returns `response.data`, every call made
    through `axiosClient` resolves to the response **body**, not the full
    `AxiosResponse` object. So in the API modules below you can use the result
    directly (e.g. `const user = await getUserById(id); user.id`) — do **not**
    read `result.data` again, or you'll get `undefined`.
...
import { getUserById, createUser } from "../api/users.api";

export const registerAndFetchUser = async (userData) => {
  // The response interceptor returns `response.data`, so the resolved value is
  // already the user object — read `.id` directly, not `.data.id`.
