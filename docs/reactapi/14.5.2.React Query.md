...
- **Key Options:**
      ...
      - `gcTime` –The time after which unused (inactive) cached data is garbage-collected. “Unused” means no active queries or components are currently subscribing to it. (In React Query v3/v4 this option was called `cacheTime`; it was renamed to `gcTime` in v5.)
...
const queryClient = new QueryClient({
  ...
  defaultOptions: {
    queries: {
      ...
      //The default value of gcTime in React Query is: 5 minutes (1000 * 60 * 5 ms)
...
To know more: [useQuery Reference](14.5.3.React Query Ref.md)
...
```txt
 ...
 │ - Status (pending...)│                  │ - Retry/failure state │
...
- Tracks query **status**: `pending`, `success`, `error` (plus a separate `fetchStatus`: `fetching`, `paused`, `idle`).
- Manages:
  - **Retries** (with exponential backoff).
  - **Garbage collection** (based on `gcTime`).
  - **Refetching policies**:
    - On **window focus**.
    - On **reconnect**.
    - On **intervals**.
...
export default function PaginatedQuery() {
  ...
  const { data, isLoading } = useQuery({
    ...
    placeholderData: keepPreviousData, // Keeps old data while fetching new (v5 replacement for keepPreviousData: true)
  });
...
// --- Suspense child ---
// In v5 the `suspense: true` query option was removed. Use the dedicated
// `useSuspenseQuery` hook instead — it always suspends until data is ready,
// so `data` is guaranteed to be defined (no loading/error states returned).
function Profile() {
...
- Global error handling via the `QueryCache` `onError` callback on the `QueryClient`.
- Per-query error handling using the returned `error` / `isError` state (the query-level `onSuccess`/`onError`/`onSettled` callbacks were **removed in v5**).
- Retry logic and exponential backoff.

> **v5 note:** `onSuccess`, `onError`, and `onSettled` no longer exist as options on `useQuery` (or `useQueries`/`useInfiniteQuery`/`defaultOptions.queries`). Read the `error`/`isError`/`data` state in your component for per-query handling, and register a `QueryCache` (or `MutationCache`) `onError` callback for global handling. Mutation-level `onSuccess`/`onError`/`onSettled` still exist on `useMutation`.
...
function PerQueryErrorExample() {
  // v5: there is no `onError` option on useQuery anymore.
  // Handle per-query errors by reading the returned `error`/`isError` state.
...
// -------------------------
// v5: query-level `onError` (including on defaultOptions.queries) was removed.
// For global query error handling, pass an `onError` callback to the QueryCache.
const queryClient = new QueryClient({
...
> 🔑 Both Fetch and Axios integrate seamlessly with React Query, as long as the function returns a promise.
...
- Expand a query to see:
  ...
  - **Status** → `pending`, `success`, or `error` (with a separate `fetchStatus`: `fetching`, `paused`, or `idle`).
