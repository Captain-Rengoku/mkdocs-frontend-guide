...
```ts
import axios, { AxiosInstance, InternalAxiosRequestConfig, AxiosResponse, AxiosError, CancelTokenSource, AxiosProgressEvent } from "axios";
...
apiClient.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    // Example: Add auth token (mutate headers, don't replace the object)
...
/* ---------------------------
   3. Parallel Requests
--------------------------- */
// Option A: axios.all() (specific to Axios — DEPRECATED, prefer Promise.all())
...
- `axios.all()` is deprecated — prefer `Promise.all()`, since it's native to JavaScript and more flexible.
*/
```
...
--------------------------- */
const authApi = axios.create({  // ⚠️ See the security note after this code block about storing tokens in localStorage.
  baseURL: "https://api.example.com",
...
!!! warning "Storing JWTs in localStorage"
    The Authentication example above reads and writes the JWT with `localStorage`
    (`localStorage.getItem("token")` / `localStorage.setItem("token", ...)`).
    This is shown here only for simplicity — `localStorage` is readable by any
    JavaScript on the page, so a single XSS vulnerability lets an attacker steal
    the token. Prefer storing the token in an **HttpOnly, Secure, SameSite cookie**
    set by the server (the `withCredentials: true` option lets Axios send it
    automatically), which is not accessible to JavaScript. See the Authentication
    chapter for the full discussion.
