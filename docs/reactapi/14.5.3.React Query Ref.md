...
const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      // every default query you want to give goes here.
    }
  }
});
...
> **Removed in v5:** the query-level `onSuccess`, `onError`, and `onSettled`
> callbacks no longer exist on `useQuery`. Read the returned `data` / `error` /
> `isError` state inside your component for per-query handling, and register a
> `QueryCache` `onError` callback on the `QueryClient` for global handling.
> (Mutation-level `onSuccess`/`onError`/`onSettled` still exist on `useMutation`.)
...
- {++ **placeholderData** (with `keepPreviousData`) ++}
      ...
      - In v5 the `keepPreviousData: true` option was removed. Import the
        `keepPreviousData` function and pass it as `placeholderData`:
...
- {++ **isPending**++}
      - `true` while the query has no data yet (`status === "pending"`).
      - This is the v5 name for the initial "no data" state.

- {++ **isLoading**++}
      - `true` while the query is fetching for the first time **and** has no data.
      - In v5 this is a derived flag equal to `isPending && isFetching`
        (the v3/v4 `isLoading` was renamed to `isPending`; `isLoading` now means
...
- {++ **refetch**++}
      ...
      - (Note: `isIdle` was **removed** from `useQuery` in v5 — the query `status`
        no longer has an `"idle"` value. A disabled query simply stays
        `"pending"`; use `fetchStatus === "idle"` to detect that it is not
        currently fetching.)

- {++ **status**++}
      - Overall status of the query: `"pending" | "error" | "success"`.
      - In v5 the `"idle"` and `"loading"` statuses were removed. Whether a
        request is currently in flight is tracked separately by `fetchStatus`
...
- {++ **removeQueries** (via `queryClient`) ++}
      - The `remove` method returned from `useQuery` was **removed in v5**.
      - To manually remove a query from the cache, use
        `queryClient.removeQueries({ queryKey })` instead.
...
- {++ **isPlaceholderData**++}
      - Boolean indicating whether the data shown is placeholder data (e.g. the
        previous page's data kept via `placeholderData: keepPreviousData`).
      - In v5 this replaces the old `isPreviousData` flag.
...
- {++ **throwOnError** → *(Boolean | (error) => boolean)* ++}
      ...
      - (Renamed from `useErrorBoundary` in React Query v5.)
...
- Resets queries back to their initial state (status `pending`, no data).  
...
- **gcTime (Number)** → Duration (in ms) inactive queries remain in cache (renamed from `cacheTime` in v5).  
...
> **Removed in v5:** the per-query `onSuccess` / `onError` / `onSettled`
> callbacks are gone here too. Read each result's `data` / `error` state, and use
> the `QueryCache` `onError` callback for global handling.
...
- **status** → `"pending"` | `"success"` | `"error"` (the v3 `"idle"`/`"loading"` values were removed; in-flight state is tracked by `fetchStatus`).  
- **isPending** → `true` if the query has no data yet.  
- **isFetching** → `true` if background fetching is in progress.  
- **isSuccess / isError** → Boolean states of the query (`isIdle` was removed for queries in v5).  
...
export default function Example() {
  const query = useInfiniteQuery({
    ...
    gcTime: 1000 * 60 * 5,            // 5 min keep unused data (renamed from cacheTime in v5)
    enabled: true,                    // Run automatically
    // `suspense: true` was removed in v5 — use the `useSuspenseInfiniteQuery` hook instead
  ...
  console.log({
    ...
    isPending: query.isPending,              // v5: replaces the old `isLoading` initial state
    isLoading: query.isLoading,              // v5: isPending && isFetching (first fetch)
    isError: query.isError,
    isSuccess: query.isSuccess,
    status: query.status,                    // pending | error | success (fetchStatus tracks in-flight)
...
- **staleTime (Number)** → Time in ms before cached data is considered stale.  
- **gcTime (Number)** → Time inactive queries stay in cache (renamed from `cacheTime` in v5).  
...
> **Removed in v5:** the query-level `onSuccess` / `onError` / `onSettled`
> callbacks were removed from `useInfiniteQuery` as well. Read the returned
> `data` / `error` state, and use the `QueryCache` `onError` callback for global
> handling.
...
- **status, data, error, isPending, isLoading, isError, isSuccess** → Same as useQuery states (v5: `isPending` is the initial no-data state; `status` is `"pending" | "error" | "success"`).  
...
function Dashboard() {
  ...
  const isFetchingByStatus = useIsFetching({
    // v5: the query status "loading" was renamed to "pending"
  ...
  return (
    <div>
      ...
      <p>Fetching by status (pending): {isFetchingByStatus}</p>
...
## 10. Prefetching (queryClient.prefetchQuery / usePrefetchQuery)

Prefetching **preloads data into the cache without rendering a component**.  
This is particularly useful for improving **performance and UX** by prefetching data before navigation or interaction.

There are two ways to prefetch in v5:

- **Imperative** — call `queryClient.prefetchQuery({ ... })` (shown below), e.g. inside a `useEffect`, an event handler, or on the server. It returns a `Promise<void>`.
- **Declarative hook** — the `usePrefetchQuery({ ... })` hook prefetches during render (it does **not** return query data); use it when you want the prefetch to be tied to a component's render rather than triggered imperatively.

The example below uses the imperative `queryClient.prefetchQuery` form.
...
## 11. Prefetching Infinite Queries (queryClient.prefetchInfiniteQuery / usePrefetchInfiniteQuery)

Prefetching an **infinite query** (a paginated query that loads data page by page) works just like prefetching a regular query, but for data consumed by `useInfiniteQuery`.

As with regular prefetching, v5 offers both an **imperative** form — `queryClient.prefetchInfiniteQuery({ ... })` (shown below, returns a `Promise`) — and a **declarative hook** — `usePrefetchInfiniteQuery({ ... })`, which prefetches during render and returns no data.

The example below uses the imperative `queryClient.prefetchInfiniteQuery` form.
...
import { HydrationBoundary } from "@tanstack/react-query"; // v5: renamed from `Hydrate`
import { QueryClientProvider } from "@tanstack/react-query";
...
2. **`HydrationBoundary` component**  
   ...
   - (Renamed from `Hydrate` in React Query v5.)

### 🔄 Workflow

1. **Server**: Prefetch queries → `dehydrate` → pass dehydrated state as props.  
2. **Client**: Wrap app in `<HydrationBoundary>` with the dehydrated state → Queries are instantly available without refetch.

### ⚙️ Props for `<HydrationBoundary>`
