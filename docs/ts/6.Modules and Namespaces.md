...
- TypeScript uses module resolution strategies to locate imported modules:
  - `"node"` (aka `node10`) — the classic Node CommonJS lookup; now legacy
  - `"node16"` / `"nodenext"` — modern Node resolution that honors `package.json`
    `"exports"`/`"imports"` and the ESM-vs-CJS distinction (use these for current
    Node.js projects)
  - `"bundler"` — for projects built with a bundler (Vite, esbuild, webpack, etc.);
    supports `"exports"` without requiring file extensions on imports. **Common
    default for Vite/modern frontend setups.**
  - `"classic"` (for very old TypeScript projects; avoid in new code)
